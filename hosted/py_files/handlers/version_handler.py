...
def compare(current_ver: str = "Unknown", latest_ver: str = "Unknown", name: str = "None"):
    print(f"{COLORS.RESET}------------------")
    print(f"Local {name} installation version: {COLORS.YELLOW+COLORS.BOLD+current_ver}")
    print(f"{COLORS.RESET}Latest {name} installation: {COLORS.YELLOW+COLORS.BOLD+latest_ver}")

    if latest_ver == "Unknown":
        print(f"{COLORS.UNDERLINE}Your {name} installation is potentially outdated, but the latest version could not be checked.")
        sleep(2)
    elif (latest_ver != current_ver and latest_ver != "Unknown"):
        print(f"{COLORS.RED+COLORS.UNDERLINE}Your {name} installation IS OUTDATED! Consider updating it.")
        sleep(2)
    else:
